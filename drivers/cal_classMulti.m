...
if conf.do_evalScores
  ...
  fig_handle=figure;
  imagesc(testConf,[0,1]) ;
  title(sprintf('Confusion matrix (%.1f%% training images/cat, %.2f%% accuracy)', ...
      conf.masterNumTrain*100, mean(diag(testConf)) * 100)) ;
  set(fig_handle, 'Color', [.988, .988, .988]);
  axis_handle = get(fig_handle, 'CurrentAxes');
  set(axis_handle, 'XTick', 1:length(classNames));
  set(axis_handle, 'XTickLabel', esti_categories);
  set(axis_handle, 'YTick', 1:length(classNames));
  set(axis_handle, 'YTickLabel', actual_categories);
  axis equal ;
  axis tight ;
end