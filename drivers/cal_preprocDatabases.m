...
contents = dir(fullfile(conf.calDir, '*.bmp')) ;
clear imdb ;
...
for ci=1:length(contents)
  ...
  mo = regexp(name, '(?<class>[\w_]+)_(?<sequence>\w+)_(?<image>\w+)\.bmp', 'names') ;
  linkName = [mo.class '_' mo.sequence '_' mo.image]   ;
  ...
  if ~ isfield(roidb.sequence, [mo.class '_' mo.sequence])
       roidb.sequence.([mo.class '_' mo.sequence]) = length(fieldnames(roidb.sequence))+1 ;
  end
  ii = ii + 1 ;
  imageName{ii}  = linkName ;
  imageClass(ii) = roidb.classes.(className) ;
  imageSequence(ii)=roidb.sequence.([mo.class,'_',mo.sequence]);
end
%[imageName,index]=sort_nat(imageName,'ascend');
% ------------------------------------------------------------------
%                                                   Split train test
% ------------------------------------------------------------------
dbi = 0 ;
...
Seqlength(ci)=max(subSequence)-min(subSequence)+1;
TrainSeq=floor(conf.masterNumTrain*Seqlength(ci));
Train=sum(Seqlength(1:ci-1))+randperm(Seqlength(ci),TrainSeq);

  for k = 1:length(sel)
    ii = sel(k) ;
    info = imfinfo(fullfile(imdb.dir, [imageName{ii} '.bmp'])) ;
    ...
    if ismember(imageSequence(ii),Train) 
